...
@MainActor
let sample04Middleware = Middleware<AppState, AppActions>(id: "sample04Middleware") { context in
  ...
  case .runEffectDemoFailure:
    dispatch(0, .setEffectRunning(true), .setEffectMessage("Running failing effect..."))
    ...
    context.runTask { @MainActor in
      do {
        ...
        dispatch(0, .setEffectMessage("Failed. Check logs for details."), .setEffectRunning(false))
        context.complete()
        throw error
      }
    }

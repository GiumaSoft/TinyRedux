...
14. [Capacity & Admission Control](#14-capacity--admission-control)
15. [Generation Tracking](#15-generation-tracking)
16. [Suspend/Resume](#16-suspendresume)
17. [Logging](#17-logging)
18. [Type Aliases](#18-type-aliases)
19. [Macros](#19-macros)
20. [Concurrency Model](#20-concurrency-model)
21. [Error Flow](#21-error-flow)
...
**Ordering caveat — `.deferred` weakens FIFO.** The worker's admission slot is released when the synchronous pipeline segment returns, which for `.deferred` happens as soon as the task is spawned: the worker then reads the next queued action. A later action can therefore traverse middlewares and reducers **before** the deferred one resumes into its own reduce chain. FIFO ordering is guaranteed for the synchronous pipeline segments only. When the resumed work must be ordered with respect to subsequent actions, the deferred body should end by `dispatch`ing a follow-up action — a new FIFO entry — instead of resuming into the reduce chain.
...
### 12.5. The `dispatch(_:snapshot:)` Overload Pair

The same label covers two distinct snapshot APIs, disambiguated by the argument type:

| Argument | Returns | Coverage |
|---|---|---|
| `snapshot: T.Type` | one `Result<Data, Error>` (async) | every pipeline terminal (12.3) |
| `snapshot: SnapshotSpec<S>` | `AsyncStream<ReduxEncodedSnapshot>` (sync) | reduce terminals only (Section 13) |

Mnemonic: **`Type.self` → one · `SnapshotSpec(…)` → many.**
...
The streaming overload of `dispatch(_:snapshot:)` returns a **bounded** `AsyncStream` of encoded snapshots, emitted whenever an edge-trigger key changes at a reduce terminal:

```swift
let stream = store.dispatch(.armNotify(ref), snapshot: SnapshotSpec(
  CharacteristicValueSnapshot.self,
  changeOn: { $0.central.valueVersion(ref) },
  limit: .timeOrCount(.seconds(30), 100)
))
for await frame in stream { /* Result<Data, Error> */ }
```

The single-shot and the streaming overload share the capture-closure shape but are **separate implementations**: the single-shot's `deferSnapshot` fires at **every** pipeline terminal — including middleware `.exit(.done)`/`.exit(.failure)` and resolver exits, where no reducer runs. The streaming overload emits only at **reduce terminals** (where `streamChain` runs). That is correct — state mutates only in reducers, so a state-derived trigger key can only change at a reduce terminal.

### 13.2. SnapshotSpec

```swift
public struct SnapshotSpec<S: ReduxState>: Sendable {
  public enum Limit: Sendable {
    case count(UInt)                    // first N valid frames
    case time(Duration)                 // for a time window
...
- **Sendable config**: the snapshot type `T` and the key type `K` are erased at construction (baked into `encode`/`trigger`), so the spec is generic only over `S` and crosses isolation boundaries.
- **`changeOn` (edge-trigger)**: a frame is emitted only when the key changes. Choose the key to track what the snapshot captures (e.g. a `valueVersion` counter ↔ the value), or a key change emits duplicate content.
- **`emitInitial`**: emit the current state at registration time — not at the arming terminal, so the first value is not missed even when the arming action never reaches a reducer.
- **`limit` (required)**: every stream is bounded — no accidentally-infinite streams. Consumer cancel / `flush` / `suspend` / `deinit` are additional early ends.
- **Buffer**: **unbounded** — no frame loss; a slow consumer receives every frame in order. The required `Limit` bounds the stream's lifetime, so the buffer cannot grow indefinitely in normal use; keep snapshots light and limits sane.
- **Stream id**: auto-`UUID` per call; streams are independent, no dedupe.

### 13.3. Streams Registry & StreamEntry
...
extension Store.Worker {
  @MainActor final class Streams {
    ...
    nonisolated init() {}
    func register(_ entry: StreamEntry)                      // ids unique → no dedupe
    @discardableResult func unregister(id: String) -> Bool
    func finishAll()                                         // flush/suspend/deinit
  }
}
```

Unlike `Subscriptions` (a build-local inside `buildDispatchProcess()`), `Streams` is a Worker **property** — initialized before the pipeline is built, captured by `streamChain`, and reachable by `registerStream`/`finishAllStreams` so `flush`/`suspend`/`deinit` can eagerly finish active streams.

`StreamEntry` is a mutable `@MainActor` class with a **`nonisolated init`** (constructible from the nonisolated `dispatch` body; Sendable by `@MainActor` isolation):

- `lastKey: AnyHashable?` — the edge-trigger cursor (why the entry is mutable; contrast `Subscriptions.Entry`, an immutable one-shot struct).
- `remaining: UInt?` — the count bound; `nil` = time-only.
- `encoder: JSONEncoder` — one per stream, passed as an argument to `encode` so no `@Sendable` closure captures it.
- `yield` / `finish` — the continuation endpoints.

`tick(_:)` emits when the key changed and returns `true` when the entry is exhausted. A frame that fails to encode (realistic: a `NaN` float rejected by `JSONEncoder`) is yielded as `.failure`, does **not** count toward `.count`, and keeps the stream alive — one bad reading must not kill a live telemetry feed. The cursor still advances: the change was observed, the failed key is not retried.
...
let streamChain: StreamChain = {
  ...
  streams.entries.removeAll { $0.tick(readOnly) }
}
```

Called inside `reduceChain` at **both** the `.done` branch and the normal end, right after `subscriptionChain()`. Three divergences from `subscriptionChain`:

1. **Not one-shot** — `tick` emits and the entry stays; the edge-trigger prevents re-firing the same datum.
2. **No `then` → enqueue** — produces a datum (`yield`), never touches `tryEnqueue` → **no admission slot** is consumed (Section 14).
3. **No generation logic** — streams are terminated eagerly at the flush/suspend/deinit source (13.6).

`streamChain` scans all entries (one `trigger` call each) per reduce terminal — keep entry counts modest.

### 13.5. Registration & Lifecycle

The stream overload registers the entry and dispatches the arming action **in the same MainActor turn** — registration happens before the action's reduce terminal, so a state change caused by the arming action itself is not missed:

```swift
Task { @MainActor in
  worker.registerStream(entry, emitInitial: spec.emitInitial)
  worker.dispatch(actions: [action])
}
```

Termination sources (all → `finish()` + removal):

| Source | Mechanism |
|---|---|
| `.count` exhausted | `tick` returns `true` → `streamChain` removes it (after `finish()`) |
| `.time` elapsed | the time `Task` calls `continuation.finish()` → `onTermination` → `unregister` |
| `.timeOrCount` | whichever of the two fires first |
| consumer cancel | task cancellation → `onTermination` → time task cancelled + `unregister` |
| `flush` / `suspend` / `deinit` | eager `worker.finishAllStreams()` (13.6) |

`yield`/`finish` after `finish()` are documented `AsyncStream` no-ops, so races between sources are harmless.

### 13.6. Eager Finish on flush / suspend / deinit

Lazy detection inside `streamChain` is insufficient whenever the pipeline stops running: `suspend()` freezes it → `streamChain` never runs → the consumer's `for await` would hang. `Store.deinit` is worse: `dispatcher.finish()` ends the worker loop but does not touch `worker.streams` → active consumers would hang **and** the Worker would leak (`onTermination` retains it).

Therefore `flush()`, `suspend()`, and `deinit` all schedule `worker.finishAllStreams()` on the MainActor. `resume()` does **not** revive streams.

Documented minor window: a `flush()` followed by a reducing action *before* the `finishAllStreams` task runs may emit one extra valid frame. Harmless for telemetry consumers.

---

## 14. Capacity & Admission Control

### 14.1. dispatcherCapacity
...
## 15. Generation Tracking

### 15.1. Flush Semantics

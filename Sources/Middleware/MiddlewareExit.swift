...
public enum MiddlewareExit<S: ReduxState, A: ReduxAction>: Sendable {
  ...
  case task(AsyncActionHandler<S>)

  /// Defers the pipeline decision to an async task body.
  /// The closure receives a ``ResumeHandler`` callback to continue the pipeline.
  case deferred(DeferredTaskHandler<A>)

...
extension Store {
  ...
  final class Worker: Sendable {
    ...
    init(
      ...
      self.task = Task { @MainActor [weak self] in
        ...
        for await event in events {
          defer { dispatcher.consume(id: event.action.id, wasCounted: event.wasCounted) }
    ...
    /// Enqueues a snapshot-stream arming action and reports the outcome, so the
    /// caller can fail the stream eagerly when the action never enters the queue
    /// (full / suspended / terminated dispatcher) instead of leaving it armed forever.
    @Sendable
    @discardableResult
    nonisolated
    func dispatchArming(_ action: A) -> Result<Void, EnqueueFailure> {
      let result = dispatcher.tryEnqueue(
        id: action.id,
        limit: 0,
        (action: action, onSnapshot: nil)
    ...
    /// Registers a snapshot-stream entry and aligns its edge-trigger cursor to
    /// the current state: emits the current state when `emitInitial` is `true`
    /// (removing the entry if that first frame already exhausts it), otherwise
    /// primes the cursor so only subsequent changes emit.
    @MainActor
    func registerStream(_ entry: StreamEntry, emitInitial: Bool) {
      /// A zero count bound is exhausted from the start — finish without
      /// registering, so the stream emits no frame at all.
      ...
      if emitInitial {
        if entry.tick(readOnly) {
          streams.unregister(id: entry.id)
        }
      } else {
        entry.prime(readOnly)
      }
    }

    /// Eagerly finishes every active snapshot stream. Invoked by
    ...
    private func buildDispatchProcess() -> ProcessHandler<S, A> {
      ...
      let subscriptionChain: SubscriptionChain = {
        ...
        for entry in matched {
          ...
          case let .failure(error) where error != .staleGeneration:
            /// A transient rejection (e.g. buffer full) must not evaporate the
            /// one-shot watcher: re-register so the next post-reducer pass retries.
      ...
      // Stream chain — ticks active stream entries post-reducer, removes exhausted ones.
      let streamChain: StreamChain = {
        guard !streams.entries.isEmpty else {

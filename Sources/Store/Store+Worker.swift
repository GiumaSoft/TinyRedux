    ...
    private func buildDispatchProcess() -> ProcessHandler<S, A> {
      ...
      let reduceChain: ReduceChain<S, A> = { action, completion in
        for reducer in reducers {
          ...
          case .next: onLog?(.reducer(reducer.id, action, .now - start, exit))
          }
        }
        completion?(readOnly)
      }
      ...
      let resolveChain: ResolveChain<S, A> = { error, action, origin, completion in
        ...
        let chain = resolvers.reduce(resolver) { next, resolver in
          { error, action in
            ...
            switch exitStatus {
            case .defaultNext: break
            default: onLog?(.resolver(resolver.id, action, .now - start, exitStatus, error))
            }
        ...
        Task {
          ...
          do {
            ...
            await MainActor.run {
              ...
              case .exit(.done): completion?(readOnly)
      ...
      let middlewareChain: MiddlewareChain<S, A> = { _, action, completion in
        ...
        let chain = middlewares.reduce(seed) { next, middleware in
          { action in
            ...
            switch exit {
            case .task, .deferred, .defaultNext: break
            // .next, .nextAs, .resolve, .exit
            default:

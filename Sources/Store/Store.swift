...
public final class Store<S: ReduxState, A: ReduxAction>: Sendable {
  ...
  /// Terminates the dispatcher stream and eagerly finishes every active
  /// snapshot stream, so consumers' `for await` loops end and the worker is
  /// released (the stream continuations retain it until termination).
  deinit {

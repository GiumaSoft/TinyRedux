...
extension Store.Worker {
  ...
  final class Dispatcher: Sendable {
    ...
    struct TaggedActionEvent: Sendable {
      ...
      /// `true` when the enqueue incremented `counts[id]` (a `limit > 0` dispatch).
      /// `consume` decrements the counter only for counted events, so unlimited
      /// dispatches of the same id cannot corrupt the rate-limit accounting.
      let wasCounted: Bool
    }
    ...
    /// Decrements `pendingCount` (floored at zero) and — only for events whose
    /// enqueue incremented it (`wasCounted`) — `counts[id]` when the id is present.
    /// Safe across `flush()` / `suspend()` resets that clear `counts`.
    func consume(id: String, wasCounted: Bool) {
      mutex.withLock { state in
        if state.pendingCount > 0 {
          state.pendingCount -= 1
        }
        guard wasCounted, let current = state.counts[id] else { return }
        if current <= 1 {

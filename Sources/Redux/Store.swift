...
public final class Store<S, A> where S : ReduxState, A : ReduxAction {
  ...
  var pendingActionsDescription: String {
    pendingActions.elements.map { ".\($0.description)" }.joined(separator: ",")
  }
}
...
extension Store {
  ...
  private func decrementCount(for action: A) {
    guard let count = pendingActionsCount[action] else { return }
    if count <= 1 {
      pendingActionsCount.removeValue(forKey: action)
    } else {
      pendingActionsCount[action] = count - 1
    }
  }
}

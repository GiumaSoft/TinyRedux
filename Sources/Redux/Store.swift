...
public final class Store<S, A> where S : ReduxState, A : ReduxAction {
  ...
  ///
  private var actionBuffer: FIFOBuffer<A>
  ...
  var pendingActionsDescription: String {
    actionBuffer.elements.map { ".\($0.description)" }.joined(separator: ",")
  }
}
...
extension Store {
  ...
  private func runDispatcher() {
    ...
    while let action = actionBuffer.dequeue() {

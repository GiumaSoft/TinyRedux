...
extension Store {
  ...
  func buildDispatchProcess() -> @MainActor (A) -> Void {
    ...
    let reduce: (A) -> Void = { [unowned self] action in
      ...
      let reduce: (Reducer<S, A>, A, @escaping @MainActor @Sendable () -> Void) -> Void = { reducer, action, complete in
        ...
        for reducer in self.reducers {
          measurePerformance { runTime in
            reduce(reducer, action) {
              onLog(.reducer(reducer.id, action, runTime()))
            }
          }
        }
      } else {
    ...
    let resolve: (any Error, A, ReduxErrorOrigin) -> Void = { [unowned self] error, action, origin in
        ...
        if let onLog = self.onLog {
          self.measurePerformance { runTime in
            ...
            case .middleware(let middlewareId):
              onLog(.middleware(middlewareId, error, action, runTime()))
            }
          }
        }
        return
      ...
      let resolversChain: (any Error, A) -> Void = self.resolvers.reduce(
        ...
        { [unowned self] error, action in
          var doNext = true
          let runProcess: (@escaping @MainActor () -> Void) -> Void = { complete in
          ...
          if let onLog = self.onLog {
            self.measurePerformance { runTime in
              runProcess() {
                switch origin {
                case .middleware(let middlewareId):
                  onLog(.resolver(resolver.id, middlewareId, error, action, runTime()))
                }
              }
            }
          } else {
      ...
      let middlewaresChain: (A) -> Void = self.middlewares.reduce(
        ...
        { [unowned self] action in
          ...
          if let onLog = self.onLog {
            self.measurePerformance { runTime in
              runProcess() {
                onLog(.middleware(middleware.id, nil, action, runTime()))
              }
            }
          } else {
            runProcess({ })

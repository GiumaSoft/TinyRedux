...
/// A main-actor store that queues actions and applies middleware and reducers sequentially. It owns
/// the mutable state instance, exposes a read-only projection through dynamic member lookup, and
/// provides dispatch helpers for single or batched actions. Actions are buffered to preserve
/// ordering and optionally throttled by per-action limits. The store builds a pipeline that runs
/// middleware, optionally resolves errors, and finally applies reducers. Logging hooks can measure
/// execution time for each stage. The dispatcher loop ensures only one action sequence runs at a
/// time on the MainActor, keeping UI observation consistent and deterministic.
@MainActor
@dynamicMemberLookup

...
/// AnyResolver
///
/// Type-erased ``Resolver`` stored as a closure. Holds heterogeneous resolvers in one
...
public struct AnyResolver<S, A>: Resolver
where S: ReduxState, A: ReduxAction
{
  ...
  let handler: ResolveHandler<S, A>

  /// Creates a type-erased resolver from a closure.
  public init(id: String,
              run handler: @escaping ResolveHandler<S, A>)
  {
    self.id = id
...
public extension AnyResolver
{
  ...
  where R: Resolver
  {
    ...
    self.handler = { global in
      ...
      let local = ResolverContext<R.S, R.A>(

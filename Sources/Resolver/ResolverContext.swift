...
@frozen
public struct ResolverContext<State: ReduxState, Action: ReduxAction> {
    ...
    public typealias Next = @MainActor (SendableError, Action) -> Void
    ...
    /// Origin of the error in the pipeline.
    public let origin: Origin

    /// Enqueues one or more actions for future dispatch.
    public let dispatch: Dispatch

    private let _next: @MainActor (SendableError, Action) -> Void
    ...
    init(
        ...
        _next: @escaping @MainActor (SendableError, Action) -> Void
    ) {
        self.state = state
        self.action = action
        self.error = error
    ...
    public var args: (State.ReadOnly, Dispatch, Next, Origin, SendableError, Action) {

...
- **Statement**: Root-level functions and direct class/struct/enum
  declarations MUST have up to DocC format comments describing
...
### FMT-002 — Parameter indentation (> 2 params or > 80 cols)

- **Level**: MUST
- **Statement**: Functions with more than 2 parameters or where 
  declaration > 80 cols MUST break the signature with one parameter 
  per line. The closing parenthesis MUST be aligned with the `func`
  keyword. Generic constraints MUST use a `where` clause on the line
  after the return type, not inline syntax.
- **Example**:
  ```swift
  func _foo<S, Content, Label>( _ title: S,
                                content: () -> Content,
                                icon: () -> Label ) 
...
- **Statement**: A screen (root view attached to `WindowGroup`,
  ...
  - `Sample+ViewModel` — Decouples long property paths into a
    ...
    Never declare as private.

### VW-003a — ViewModel responsibility

- **Level**: MUST
- **Statement**: A ViewModel (`Sample+ViewModel`) MUST only contain
  logic that shapes application state into values the View can
  consume directly. This includes short-path aliases, filtering,
  sorting, search predicates, and binding adapters. Domain-level
  interpretation of raw data (decoding, encoding, formatting
  protocol-specific values) MUST NOT live in the ViewModel; it
  MUST be implemented as standalone functions under `Functions/`.
- **Rationale**: Keeping ViewModels thin ensures that data
  interpretation logic is reusable by any consumer (views, loggers,
  exports) and that ViewModels remain simple adapters between
  application state and the view layer.
- **Compliant examples**:
  ```swift
  // Alias — shortens a property path
  var identifier: String {
    result.identifier.uuidString
  }

  // Filtering — shapes a collection for the view
  var connectablePeripherals: Array<DiscoveredResult> {
    searchedSortedPeripherals
      .filter { $0.isConnectable }
  }

  // Search predicate — combines user input with data
  var searchedSortedPeripherals: Array<DiscoveredResult> {
    ...
    return if searchText.isEmpty {
      ...
      sortedPeripherals
        .filter {
          let matchIdentifier = $0.identifier.uuidString
            .lowercased()
            .contains(searchText)
          let matchLocalName = $0.name?
            .lowercased()
            .contains(searchText)

          return (matchIdentifier || (matchLocalName ?? false))
        }
    }
  }
  ```
...
| VW-003a | ViewModel responsibility | MUST |

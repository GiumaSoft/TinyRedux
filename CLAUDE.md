...
- **Middleware**: `Identifiable`, `Sendable`
  Side-effects. `run()` throwing-Sync su MainActor, ritorna `MiddlewareExit`. Async via `.task` (fire-and-forget) o `.deferred` (resume-based).
...
- **Store.Log**
  ...
  deferred resume converte `MiddlewareResumeExit` → `MiddlewareExit` via `init(from:)`.
...
- **StatedMiddleware**
  `@frozen` struct, conforma a `Middleware`. `@MainActor` inherited dal protocol.
  Cattura un coordinator (`AnyObject & Sendable`) per side-effects stateful cross-dispatch.
  Init: `init(id:coordinator:handler:)`.
...
2. Pipeline (interamente MainActor, fold-based):
   ...
   middlewareChain(action)
     ...
     │     ├── .deferred(handler)  → handler(resume) — pipeline sospesa, completion threaded
...
2. **Log/timing**: automatico per tutti i componenti (middleware, reducer, resolver).
   ...
   Per `.task`/`.deferred`: solo log async (durata task/resume), nessun sync log.
...
5. **Dispatch con completion**: nonisolated, callback sullo state risultante.
   ...
   Per `.deferred`: la completion è catturata nella closure del resume, invocata solo dopo che il resume completa la pipeline.
...
public enum MiddlewareResumeExit<A: ReduxAction>: Sendable {
  ...
  case exit(Result<Void, SendableError>)                            // uscita forzata dalla pipeline
}
...
public enum MiddlewareExit<S: ReduxState, A: ReduxAction>: Sendable {
  ...
  case deferred(@Sendable (@escaping ResumeHandler<A>) -> Void)  // handler riceve resume
...
public enum ResolverExit<A: ReduxAction>: Sendable {
  ...
  case exit(Result<Void, SendableError>)  // .success → gestito, .failure → non gestibile
}
...
### 6. Deferred + Resume (API Separation)

`resume` non è sul context. Esiste solo come parametro della closure in `.deferred`:

```swift
return .deferred { resume in
  Task {
    let user = try await api.fetchUser()
    resume(.nextAs(.setUser(user)))  // .next per action corrente
  }
}
```

Compiler impedisce `resume` fuori da `.deferred`. `MiddlewareResumeExit` non ha `.deferred` né `.task`.
...
│   ├── .deferred(handler) → handler(resume) — pipeline sospesa, completion threaded
│   │                        resume(.next)           → Middleware B → seed → completion
│   │                        resume(.nextAs(a2))     → Middleware B (modified) → seed → completion
│   │                        resume(.resolve(e))     → resolveChain(..., completion)
│   │                        resume(.exit(.success)) → reduceChain(action, completion)
│   │                        resume(.exit(.failure)) → completion?(readOnly)

...
extension TinyReduxTests {
  ...
  /// StatedMiddleware captures a coordinator object that persists across dispatch cycles, enabling stateful
  /// side effects like counters, caches, or network session managers. The coordinator is injected at init
  /// and passed as the first parameter to the handler closure on every invocation, decoupling state ownership

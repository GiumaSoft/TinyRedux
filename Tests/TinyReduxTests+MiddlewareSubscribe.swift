...
extension TinyReduxTests {
  ...
  /// A matched subscription whose action is rejected by a full dispatcher must
  /// survive: the entry is re-registered and retried at the next post-reducer
  /// pass once capacity frees (M3). Before the fix the one-shot watcher
  /// evaporated silently on `bufferLimitReached`.
  ///
  /// Capacity 2 choreography: while `.inc` #1 is processed, its own slot plus
  /// the queued `.inc` #2 saturate the dispatcher, so the subscription match at
  /// #1's reduce terminal is rejected; at #2's terminal one slot is free and
  /// the retried match enqueues `.run`.
  @Test
  func subscriptionSurvivesEnqueueRejectionAndRetries() async {
    let state = TestState()
    let subscribed = Mutex<Bool>(false)

    let middleware = AnyMiddleware<TestState, TestAction>(id: "m") { context in
      let isFirstRun = context.action == .run && !subscribed.withLock { $0 }
      if isFirstRun {
        subscribed.withLock { $0 = true }
        context.subscribe(
          when: { $0.value >= 1 },
          then: { .run }
        )
    ...
    let reducer = AnyReducer<TestState, TestAction>(id: "r") { context in
      switch context.action {
      case .inc: context.state.value += 1
      case .run: context.state.log.append("run")
      }
      return .next
    }
    let store = Store(
      ...
      resolvers: [],
      reducers: [reducer],
      options: StoreOptions(dispatcherCapacity: 2)
    )

    let _ = await store.dispatch(.run, snapshot: TestSnapshot.self)
    #expect(state.log == ["run"])

...
extension TinyReduxTests {
  ...
  /// A resolver returning `.exit(.failure(error))` declares the error unrecoverable, so a
  /// `dispatch(_:snapshot:)` caller must receive `.failure(error)` — mirroring the middleware
  /// chain's `.exit(.failure)` terminal. Before the M1 fix the resolve chain resumed the snapshot
  /// continuation with `.success(readOnly)` for every `.exit` payload, masking the failure.
  /// The sibling expectations pin the `.exit(.success)` / `.exit(.done)` paths to `.success`.
  @Test
  func resolverExitFailureDeliversFailureToSnapshotCaller() async {
    let state = TestState()

    let middleware = AnyMiddleware<TestState, TestAction>(id: "throw-mw") { context in
      throw TestError.test
    }
    let failingResolver = AnyResolver<TestState, TestAction>(id: "unrecoverable") { context in
      return .exit(.failure(context.error))
    }
    let store = Store(
      initialState: state,
    ...
    let result = await store.dispatch(.run, snapshot: TestSnapshot.self)

    guard case .failure(let error) = result else {
      Issue.record("expected .failure from snapshot dispatch, got .success")
      return
    }
    #expect(error as? TestError == .test)

    /// Sibling: `.exit(.success)` and `.exit(.done)` still resume the caller with `.success`.
    for exit in [ResolverExit<TestAction>.exit(.success), .exit(.done)] {
      let recoveringResolver = AnyResolver<TestState, TestAction>(id: "recovering") { _ in exit }
      let recoveringStore = Store(
        initialState: TestState(),
        middlewares: [middleware],
        resolvers: [recoveringResolver],
        reducers: []
      )

      let recovered = await recoveringStore.dispatch(.run, snapshot: TestSnapshot.self)

      #expect(throws: Never.self) { try recovered.get() }
    }
  }

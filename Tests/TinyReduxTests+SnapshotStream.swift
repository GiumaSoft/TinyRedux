...
extension TinyReduxTests {
  ...
  /// A zero count bound is exhausted from the start: `registerStream` finishes
  /// the stream without registering the entry and no frame is emitted — even
  /// with `emitInitial` (M7). Exercised on the internal entry directly because
  /// `SnapshotSpec` asserts a positive count bound in debug builds.
  @Test
  func streamCountZeroEmitsNoFrameAndIsNotRegistered() async {
    let store = Self.makeStreamStore()
    let yielded = Mutex<Int>(0)
    let finished = Mutex<Bool>(false)

    let entry = Store<TestState, TestAction>.Worker.StreamEntry(
      id: "zero-bound",
      trigger: { AnyHashable($0.value) },
      encode: { readOnly, encoder in try encoder.encode(TestSnapshot(state: readOnly)) },
      yield: { _ in yielded.withLock { $0 += 1 } },
      finish: { finished.withLock { $0 = true } },
      remaining: 0
    )

    store.worker.registerStream(entry, emitInitial: true)

    #expect(finished.withLock { $0 })
    #expect(yielded.withLock { $0 } == 0)
    #expect(store.worker.streams.entries.isEmpty)
  }
}

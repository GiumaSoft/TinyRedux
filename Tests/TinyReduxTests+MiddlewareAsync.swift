...
extension TinyReduxTests {
  ...
  /// CHARACTERIZATION (M6) — `.deferred` weakens FIFO: while the handler is
  /// awaited the worker advances, so a later action reduces before the deferred
  /// one resumes into its own reduce chain. Pins the documented contract
  /// (`MiddlewareExit.deferred`, ARCHITECTURE §7); a future move to strict FIFO
  /// must change this test deliberately.
  @Test
  func deferredResumeReducesAfterLaterActions() async {
    let state = TestState()
    let release = Mutex<Bool>(false)

    let middleware = AnyMiddleware<TestState, TestAction>(id: "defer-mw") { context in
      guard context.action == .run else { return .defaultNext }
      return .deferred { _ in
        /// Hold the resume until the test has observed the later action's reduce.
    ...
    let reducer = AnyReducer<TestState, TestAction>(id: "order") { context in
      context.state.log.append(context.action.id)
      return .next
    }
    let store = Store(
      initialState: state,
    ...
    /// The later `.inc` reduces while `.run` is still suspended in its handler.
    await Self.poll { state.log.isEmpty }
    #expect(state.log == ["inc"])

...
struct TinyReduxTests {
    ...
    func onLogCallbackReceivesMiddlewareLog() async {
        ...
        for _ in 0..<500 where logEntries.withLock({ $0.count }) < 2 {
            try? await Task.sleep(nanoseconds: 2_000_000)
        }
    ...
    func onLogCallbackReceivesResolverLog() async {
        ...
        for _ in 0..<500 where !resolverLogReceived.withLock({ $0 }) {
            try? await Task.sleep(nanoseconds: 2_000_000)
        }
    ...
    func completeEmitsOneLogPerComponent() async {
        ...
        let entries = logEntries.withLock { $0 }
        let middlewareLogs = entries.filter { if case .middleware = $0 { return true }; return false }
        let reducerLogs = entries.filter { if case .reducer = $0 { return true }; return false }
    ...
    func completeEmitsOneLogPerResolver() async {
        ...
        for _ in 0..<500 where logEntries.withLock({ $0.isEmpty }) {
            try? await Task.sleep(nanoseconds: 2_000_000)
        }
        ...
        let entries = logEntries.withLock { $0 }
        let resolverLogs = entries.filter { if case .resolver = $0 { return true }; return false }
    ...
    func middlewareTaskRunsOffMainActor() async {
        ...
        for _ in 0..<500 where isMain.withLock({ $0 }) == nil {
            try? await Task.sleep(nanoseconds: 2_000_000)
        }

...
extension TinyReduxTests {
  ...
  /// `consume(id:wasCounted:)` decrements `pendingCount` and is safe even when `counts[id]`
  ...
  func consumeDecrementsPendingCountAndSurvivesCountsReset() {
    ...
    dispatcher.consume(id: "x", wasCounted: false)
    dispatcher.consume(id: "x", wasCounted: false)
  ...
  /// Unlimited (`limit: 0`) dispatches of an id must not corrupt the rate-limit
  /// accounting of concurrent limited dispatches of the same id: `consume`
  /// decrements `counts[id]` only for events whose enqueue incremented it (M4).
  /// Before the fix, consuming the unlimited event dropped `counts["x"]` to 1
  /// and a third limited enqueue slipped past the cap.
  @Test
  func unlimitedConsumeDoesNotCorruptLimitedAccounting() {
    let dispatcher = Store<TestState, TestAction>.Worker.Dispatcher(capacity: 256)

    /// Two limited slots taken — `counts["x"]` is at its cap of 2.
    _ = dispatcher.tryEnqueue(id: "x", limit: 2, (action: TestAction.inc, onSnapshot: nil))
    _ = dispatcher.tryEnqueue(id: "x", limit: 2, (action: TestAction.inc, onSnapshot: nil))
    /// One unlimited dispatch of the same id, enqueued and fully consumed.
    _ = dispatcher.tryEnqueue(id: "x", limit: 0, (action: TestAction.inc, onSnapshot: nil))
    dispatcher.consume(id: "x", wasCounted: false)
    ...
    guard case .failure(.maxDispatchableReached) = result else {
      Issue.record("expected .failure(.maxDispatchableReached), got \(result)")
